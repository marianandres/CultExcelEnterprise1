...
// esto es la clave de la sustituci�n en los archivos parseados
$uniq = uniqid("mkI");
...
if (!$argv[0]) { // {{{ parte web
  ...
  if (!$ok) {
    ...
    if ($pwpass)
      if ($wpass)
        die($html1 . sprintf($html2, __('Contrase�a')));
      else
        die(sprintf(__('Error en %s'), $ipwfn));
    else
    if ($wpass = $_REQUEST['pass']) {
      fwrite($f = fopen("$pwfn$uniq.php", 'w'), "<?php //!!\n\$wpass=" . var_export($wpass, true) . '; ?>');
      fclose($f);
      chmod("$pwfn$uniq.php", 0600);
      $ok = true;
    } else
      die($html . sprintf($html2, __('Nueva contrase�a')));
  }
  if ($ok) {
    if (!($fout = $_REQUEST['fout'] or $xcml = $_REQUEST['xcml'])) {
      echo $html1 .
      "<form target=ifr action\"={$_SERVER['SCRIPT_NAME']}\" method=post>" .
      __('L�nea de comandos:') . ' <input name=cml size=80><input type=submit name=xcml value="' . __('Proceder') .
...
foreach (array("gzdeflate" => "zlib", uniqid => "misc",
 ...
 "pg_connect" => "PostgreSQL") as $f => $p)
  if (!function_exists($f))
    logea(sprintf(__("La funci�n %s no est� disponible, revisa el modulo php %s\nEs posible que mkInstaller no funcione correctamente"), $f, $p));
...
function mkdirR($dir, $mode = 0755) {
  if (is_dir($dir) || (@mkdir($dir) && @chmod($dir, $mode)))
    return true;
...
function filtra($cfil, $nfil, $bf) {
  ...
  $bcfil = cojebuf($cfil);
  ob_start();
  if (!@eval('return true; function ' . ($fcfil = $uniq . rand() . rand()) . '($inbuff,&$outbuff) {?>' . $bcfil . '<?php }') and ! @eval('return true; function ' . ($fcfil = $uniq . rand() . rand()) . '($inbuff,&$outbuff) {?>' . $bcfil . '}'))
    error(sprintf(__("*** La carga de %s ha producido un error sint�ctico"), $cfil) . ob_get_clean(), $ncte);
  if ($fcfil($bf, $nbf)) {
    logea(sprintf(__("Parseado %s con %s"), $nfil, $cfil));
    return $nbf;
  } else {
    logea(sprintf(__("*** La ejecuci�n de %s ha producido un error, %s no filtrado, sin cambios"), $cfil, $nfil));
    return $bf;
  }
}
...
function parseopt($opt, $opts) {
  ...
  if ($cu == 0)
    error(__('Las opciones v�lidas son: ') . implode(' ', $opts) . '. ' .
            sprintf(__("Opci�n desconocida '%s'"), $opt));
  if ($cu > 1)
    error(__('Opci�n ambigua') . $opt);
  return $opt1;
}
...
while ($cont) {
  ...
  switch ($para) {
    ...
    case '-c':
      if ($inst)
        error(__('S�lo un fichero de instalaci�n'));
      $inst = $argv[$ia];
      $ia++;
      break;
    case '-D':
    ...
    case '-v':
      $skipv = true;
      logea(sprintf(__('Versi�n aceptada: %s'), $mver));
      break;
    case '-r':
      $reco = true;
      $ndi = $argv[$ia];
    ...
    case '-n':
      ...
      switch ($opt1) {
        ...
        case 'tpl':
          if ($plan)
            error(__('S�lo un nombre de plantilla, deben existir: en-fichero , es-fichero ...'));
...
if (!$dtod and ! $skipv and ! $cml and $mver)
  switch (@file_get_contents("http://expire.nisu.org/?prg=mkInstaller.php&ver=$mver")) {
    case 'UP': logea(__('Esta versi�n de mkInstaller tiene problemas graves, debe ser actualizada'));
      reInMk();
      die();
    case 'NW': logea(__('Disponible una nueva versi�n de mkInstaller'));
      reInMk();
      break;
    case 'RE': logea(__('Disponible una nueva versi�n de mkInstaller, puede usar -D para instalarla'));
      break;
  }
// }}}
...
//}}}
// {{{ carga del fichero de instalaci�n
$defvars = array();
$myopt = array();
$svbufs = array();
$defbdds = array();
if (file_exists($inst)) {
  // debe ser c�digo v�lido php y con la marca
  $cinst = file_get_contents($inst);
  if (substr($cinst, 0, 10) != "<?php //!!")
    error(sprintf(__("El fichero '%s' no es v�lido"), $inst));
  eval("?>$cinst<?php ");
  if (!($defvars or $defbdds or $svbufs))
    error(sprintf(__("Problemas en el c�digo fuente de %s, ed�talo a mano"), $inst));
  if ($usim) {
    $defbdds = array();
    $svbufs = array();
  }
...
if ($vUrl = $myopt['ver']) {
  ...
  if (!$ver)
    error(sprintf(__('No puedo obtener la versi�n actual a partir de %s'), $vUrl), $ncte);
  else
    logea(sprintf(__('Versi�n actual obtenida: %s'), $ver));
}
// }}}
// {{{ c�digo de lectura de bufers
// el lio del trim es para prevenir errores internos mios
$deftk = 'function tkInBuff($po) {
...
foreach ($buf as $nf => $b) {
  ...
  if ($b['p']) { // {{{ es parseable
    ...
    if ($nf == $nbu2 and ! $b['x'] and ( realpath($nf) != getcwd() . "/$nf"))
      logea(sprintf(__("** La inclusi�n de nombres de fichero no-relativos puede ser peligrosa: %s"), $nf));
    if (!$bf = filtra($b['f'], $nf, $bf))
      continue;
    // limpio zonas de c�digo oculto
    $bf = preg_replace('+(\s*)//\*(.*?)//\*.*?$+mes', '" // ".(($lcem=count(explode("\n",\'\1\2\'))-1 and $tlcem+=$lcem or $tcem++) ?
	sprintf(__("%d l�neas de c�digo de depuraci�n eliminadas por mkInstaller"),$lcem) :
	__("Un comentario eliminado por mkInstaller"))', $bf);
    ob_start();
    if (!@eval("return true; function $uniq" . rand() . rand() . "() {?> $bf <?php }") and ! @eval("return true; function $uniq" . rand() . rand() . "() {?> $bf }") and ! $istx)
      error(sprintf(__("*** La carga de %s ha producido un error sint�ctico"), $nf) . ob_get_clean(), $ncte);
    ob_end_clean();
    ...
    // limpia el c�digo original
    // esto es necesario para usar str_replace despu�s con trankilidad
    $bf = preg_replace('+^(\s*//!\s*\n(?:\s*// *%..%: *[^\n]*\n)*\s*)(\$.*?)(\s*)=(\s*).*?(\s*)(;\s*?)$+sm', "\\1\\2\\3=\\4$uniq.\\2.$uniq\\5\\6", $bf);
    ob_start();
    if (!@eval("return true; function $uniq" . rand() . rand() . "() {?> $bf <?php }") and ! @eval("return true; function $uniq" . rand() . rand() . "() {?> $bf }") and ! $istx)
      error(sprintf(__("*** El procesamiento de %s ha producido un error sint�ctico"), $nf) . ob_get_clean(), $ncte);
    ob_end_clean();
    if ($b['x'] == 'n')
      unset($bu2[$ib2]);
    else if ($b['x'] == 'f')
    ...
    if ($fus) {
      ...
      $load = uniqid('f');
      $fu = preg_replace('/.*(function) (mmIniStr.*?}).*/s', "\\1 $load\\2 \$ids=$load$llam;", $fus);
      @eval($fu);
  ...
  // {{{ no parseable
    ...
    if ($b['x'] or $b['j']) {
      if (!$bf = filtra($b['f'], $nf, cojebuf($nf)))
        continue;
      ob_start();
      if (!@eval("return true; function $uniq" . dechex(crc32($nf)) . "() {?> $bf <?php }") and ! @eval("return true; function $uniq" . dechex(crc32($nf)) . "() {?> $bf }"))
        error(sprintf(__("*** La carga de %s ha producido un error sint�ctico"), $nf) . ob_get_clean(), $ncte);
      ob_end_clean();
      $bu2[$ib2]['po'] = intW($bf);
    }
...
if ($tlcem)
  logea(sprintf(__("%d l�neas de c�digo de depuraci�n eliminadas por mkInstaller"), $tlcem));
if ($tcem)
...
foreach ($vars as $var => $kk) {
  // a�ade las vars no definidas en el fich $inst y presentes en los fuentes
  if (!$defvars[$crc = dechex(crc32($var))]) {
    logea(sprintf(__("A�ado variable %s al fichero de definiciones"), $var));
...
// mira los de defvars que ya no est�n en las fuentes y da warning
// y sobreescribe el 'df' de las variables con 'cp'
foreach ($defvars as $idx => $def)
  if (!array_key_exists($def['va'], $vars)) {
    if (!$usim)
      logea(sprintf(__("** La variable %s ya no est� en las fuentes"), $def['va']));
  }
  else if ($def['cp']) {
...
// }}}
// {{{ idiomas
// averigua los idiomas del fich de instalaci�n
// el fichero $inst puede tener idiomas que no est�n en los fuentes
$tids = array();
foreach ($defvars as $def)
  foreach ($def['lg'] as $id => $kk)
    $tids[$id] = true;

// a�ade los idiomas de los fuentes a los definidos a mano
...
// completa en $defvars los idiomas que no est�n
...
foreach ($cdefbdd as $icdefbdds) {
  ...
  if ($ok)
    ...
    logea(sprintf(__('** Definici�n de base de datos %s ignorada, ya existe una igual'), $icdefbdds));
} // }}}
// {{{ mensajes internos del instalador
$iipgmg = array(
    'Lfne' => array(
        'es' => "La funci�n %s no est� disponible, revisa el modulo php %s",
        'ca' => "La funcio %s no esta disponible, revisa el m�dul php %s",
...
$pgmg = array(
    'Aeuv' => array(
        'es' => 'Atenci�n: Existe una versi�n actualizada de este instalador.',
        'ca' => 'Atenci�: Existeix una versi� actualitzada de aquest instal.lador.',
    ...
    'Atli' => array(
        'es' => 'Lea tambi�n la licencia del instalador:',
        'ca' => 'Legiu tamb� la llicencia del instal.lador:',
    ...
    'Dppa' => array(
        'es' => 'Deben pasarse par�metros',
        'ca' => 'Deuen de pasarse parametres',
        'en' => 'Some parameters must be passed',),
    'Didl' => array(
        'es' => 'Debe instalarse desde la misma IP que se instal� la �ltima vez',
    ...
    'Dyul' => array(
        'es' => 'Descargar y usar la nueva versi�n',
        'ca' => 'Descarregar i emprar la nova versi�',
    ...
    'Eefs' => array(
        ...
        'en' => 'Running file %s',),
    'Einp' => array(
        'es' => 'Este instalador no deber�a ejecutarse en \'safe_mode\' pues necesita crear subdirecotrios con archivos.',
    ...
    'Eval' => array(
        ...
        'en' => 'Evaluate input',),
    'ExIn' => array(
        'es' => 'Expresi�n incorrecta',
        'ca' => 'Expressi� incorrecta',
    ...
    'Inco' => array(
        'es' => 'Instalaci�n de ficheros y bdd completa',
        'ca' => 'Instalaci� de arxius i bdd completa',
    ...
    'Inst' => array(
        ...
        'en' => ' Install ',),
    'Lvsu' => array(
        'es' => 'La variable %s usada en la def de bdd %s no est� definida o no tiene valor actual',
        'ca' => 'La variable %s usada en la def de la bdd %s no est� definida o no te valor actual',
    ...
    'Nspu' => array(
        'es' => 'No se pudo usar la nueva versi�n',
        'ca' => 'No se ha pogut emprar la nova versi�',
...
if (!$nam = $myopt['nam']) {
  $nam = $uniq;
  $myopt['nam'] = $nam;
  logea(__("** Ponga un nombre a su aplicaci�n"));
}
// }}}
// {{{ licencias
// licencia instalador
if ($ili = $myopt['ili'])
  ...
  $ili = $milic;
  logea(__("** Licencia del instalador por defecto, deber�a usar -o ili"));
}
...
if ($lic = $myopt['lic']) {
  ...
  $hay = false;
  // determina el idioma de licencia m�s apropiado
  foreach ($tids as $id => $kk)
  ...
  if (!$hay)
    error(sprintf(__("El fichero %s %s no corresponde con ning�n idioma definido"), __('licencia'), $lic), $ncte);
  foreach ($tids as $id => $kk) {
  ...
  logea(__("** Deber�a especificar una licencia con -o lic"));
...
if ($plan = $myopt['tpl']) {
  ...
  $hay = false;
  // determina el idioma de plantilla m�s apropiado
  foreach ($tids as $id => $kk)
  ...
  if (!$hay)
    error(sprintf(__("El fichero %s %s no corresponde con ning�n idioma definido"), __('plantilla'), $plan), $ncte);
  if ($pid != $idpr)
  ...
  foreach ($tids as $id => $kk) {
    if (file_exists($nf = preg_replace("+(^|.*/)$pid(-[^/]*)$+", "\${1}$id\\2", $plan))) {
      ...
      foreach ($ma[1] as $f) {
        // antes cargaba s�lo im�genes locales, es mejor cargar todas para evitar que dejen de existir
      ...
      $p = str_replace(array('{NAM}', '{IVER}'), array($nam, $mver), $p);
      if ($ver)
  ...
  $plat = '	<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">
	<html>
	<head>
...
';
  $imgsai = '$imgs=unserialize(tkInBuff(' . $imgsai . '));
  ';
} else {
  $imgsa = '';
  $imgsai = '';
...
if ($defbdds) {
  ...
  if ($drop)
    if ($dtod and ! $usim) {
      logea(__("La opci�n 'drp' ha quedado desactivada"));
      $drop = false;
    } else
      logea(__("** Usando la opci�n peligrosa 'drp' => drop tables"));
  $igno = $myopt['idb'];
  foreach ($defbdds as $idb => $bd) {
    ...
    foreach (array('ho', 'pt', 'us', 'pw') as $n) {
      if ($bd[$n] and $bd["v$n"]) {
        error(sprintf(__("La definici�n de bdd %s contiene %s y v%s ambos definidos"), $idb, $n, $n), $ncte);
        continue;
      }
      if ($nv = $bd["v$n"]) {
        // cojo datos para la conexi�n actual
        if ($vv = $vars[$nv])
          eval('$con[$n]=' . $vv . ';'); // nombre actual
        else {
          error(sprintf(__("La variable %s usada en la def de bdd %s no est� definida o no tiene valor actual"), $nv, $idb), $ncte);
          continue;
        }
      } else if ($bd[$n]) {
        $con[$n] = $bd[$n];
    ...
    foreach ($bd as $ibd => $n)
      if (is_array($n)) {
        if ($n['vdb'] and $n['db']) {
          error(sprintf(__("La definici�n de bdd %s de %s contiene db y vdb ambos definidos"), $ibd, $idb), $ncte);
          continue;
        }
        if ($nv = $n['vdb']) {
          if (!$vv = $vars[$nv]) {
            error(sprintf(__("La variable %s usada para el nombre de la bdd %s de %s no est� definida o no tiene valor actual"), $nv, $ibd, $idb), $ncte);
            continue;
          }
          eval('$nbd=' . $vv . ';');
        } else {
        ...
        if ($fil = $n['fi']) {
          ...
          if ($fil or $n['dm']) { // {{{
            ...
            foreach ($n['tc'] as $etab) {
              ...
              if (($itabs === false) // no es regex
                ...
                $itabs = array($etab);
              if (!$itabs) {
                logea(sprintf(__("** El patr�n '%s' no corresponde con ninguna tabla"), $etab));
                continue;
              }
          ...
          error(sprintf(__("No se especific� ni conexi�n ni archivo para la bdd %s"), $nbd), $ncte);
          continue;
        }
  ...
  // {{{ c�digo en el instalador para bdd
  $phpp.=
          '	  if (!$o["skdb"]) {
...
// }}}
// {{{ c�digo del instalador en post para el final
$phpp.=
        '	  $tby=' . $tby . '; $twr=' . $twr . ';
	  loge(st("Inco"));
	  if (!$cml)
...
$htmli = '	<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">
	<html>
	<head>
	<style type="text/css">
	{CSS}
...
// }}}
// {{{ �ltimas escrituras internas, debe hacerse antes de _mkI.def
...
// {{{ crea el _mkI...
  ...
  creadir($inst);
  if (@filesize($inst) == 0)
    @unlink($inst); // para que no se quede vac�o
...
' . //foreach(preg_split(\'/,/\',
	...
	function savePi() {
	  global $dvars, $vo, $o;
	  if ($_REQUEST["nsav"] or $o["nsav"])' . // nsv se pondr� por porgramacion con -f load o -f run
                '
	    return false;
	  $pdv=array();
...
$taphp = strlen($php);
// permite editar hasta 50k m�s de c�digo sobre el instalador construido
...
if ($fkey = $myopt['key']) {
  ...
  while (!$key = @openssl_pkey_get_private("file://$fkey", $rpwd)) {
    logea(sprintf(__("*** Fall� la carga de la llave privada con la contrase�a tomada de la variable %s, firma no generada\nEscriba la contrase�a (CON ECO): "), $pwd));
...
if ($big) {
  ...
  if ($key) {
    ...
    if (openssl_private_encrypt(serialize(array('hash' => $hash, 'time' => time())), $sig, $key)) {
      fwrite($h2 = fopen("$outd.sig", "wb"), base64_encode($sig));
      fclose($h2);
...
if ($outf) {
  ...
  if ($key) {
    $hash = sha1_file($outf);
    if (openssl_private_encrypt(serialize(array('hash' => $hash, 'time' => time())), $sig, $key)) {
      fwrite($h2 = fopen("$outf.sig", "wb"), base64_encode($sig));
      fclose($h2);
  ...
  if ($dtod) {
    ...
    if (($tam = filesize($outf) / 1024 / 1024) > 4) {
      ...
      if ($big)
        ...
        logea(__("** Considera la opci�n de usar -o big en lugar de -o out"));
    }
  }
}
...
function mmIniStr($my) {
  ...
  $lang_lang = array(
      ...
      '%d l�neas de c�digo de depuraci�n eliminadas por mkInstaller' => array(
          'ca' => '%d llinies de codig de depuraci� eliminades per mkInstaller',
          'en' => '%d lines of debug code removed by mkInstaller',),
      '%s ya existe' => array(
          'ca' => '%s ja existeix',
          'en' => '%s already exists',),
      '** Considera la opci�n de usar -o big en lugar de -o out' => array(
          'ca' => 'Considera la opci� de fer servir -o big en lloc de -o out',
          'en' => 'Consider the option of using -o big instead of -o out',),
      '** Deber�a especificar una licencia con -o lic' => array(
          'ca' => '** Deurie de especificar una llicencia de us amb -o lic',
          'en' => '** You should especify a license with -o lic',),
      '** Definici�n de base de datos %s ignorada, ya existe una igual' => array(
          'ca' => '** Definici� de base de dades %s ignorada, ja existeix una igual',
      ...
      '** El patr�n \'%s\' no corresponde con ninguna tabla' => array(
          'ca' => '** El patr� \'%s\' no es correspon amb cap taula',
      ...
      '** La inclusi�n de nombres de fichero no-relativos puede s-3032891603' => array(
          'ca' => '** La inclusi� de noms de arxiu no-relatius pot ser perillosa: %s',
          'en' => '** The inclussion of non relative filenames can be dangerous: %s',),
      '** La variable %s ya no est� en las fuentes' => array(
          'ca' => '** La variable %s ya no est� en les fonts',
          'en' => '** The variable %s is not yet in the sources',),
      '** Licencia del instalador por defecto, deber�a usar -o ili' => array(
          'ca' => '** Llicencia del instal.lador per defecte, deur�e emprar -o ili',
      ...
      '** Ponga un nombre a su aplicaci�n' => array(
          'ca' => '** Done-li un nom a la seva aplicaci�',
      ...
      '** Usando la opci�n peligrosa \'drp\' => drop tables' => array(
      ...
      '*** El procesamiento de %s ha producido un error sint�ctico' => array(
          'ca' => '*** El processament de %s ha produit una errada sint�ctica',
          'en' => '*** Processing %s has produced syntax error',),
      '*** Fall� la carga de la llave privada con la contrase�a t-1607244275' => array(
          'ca' => 'Ha fallat la c�rrega de la clau privada fent servir la contrase�a de la variable %s, la firma no se ha generat' . "\n" .
      ...
      '*** La carga de %s ha producido un error sint�ctico' => array(
          'ca' => '*** La carrega de %s ha produit una errada sint�ctica',
          'en' => '*** loading of %s has produced a syntactic error',),
      '*** La ejecuci�n de %s ha producido un error, %s no filtra-2114977031' => array(
          'ca' => '*** La ejecuci� de %s ha produit una errada, %s no ha segut filtrat, sense canvis',
      ...
      'Actualizando %s' => array(
          'ca' => 'Actualizant %s',
          'en' => 'Updating %s',),
      'A�ado variable %s al fichero de definiciones' => array(
      ...
      'Disponible una nueva versi�n de mkInstaller' => array(
          'ca' => 'Es disposa d\'una nova versi� de mkInstaller',
          'en' => 'A new version of mkInstaller is available',),
      'Disponible una nueva versi�n de mkInstaller, puede usar -D-1093888853' => array(
          'ca' => 'Es disposa d\'una nova versi� de mkInstaller, pot emprar -D per a instal.larla',
          'en' => 'A new version of mkInstaller is available, you can use -D to install it',),
      'El fichero %s %s no corresponde con ning�n idioma definido' => array(
      ...
      'El fichero \'%s\' no es v�lido' => array(
          'ca' => 'El arxiu \'%s\' no v�lid',
      ...
      'Esta versi�n de mkInstaller tiene problemas graves, debe s-3989198745' => array(
          'ca' => 'Esta versi� de mkInstaller te problemes greus, deu de ser actualitzada',
      ...
      'La definici�n de bdd %s contiene %s y v%s ambos definidos' => array(
          'ca' => 'La definici� de bdd %s cont� %s i v%s els dues definits',
          'en' => 'The definition of db %s contains %s and v%s both defined',),
      'La definici�n de bdd %s de %s contiene db y vdb ambos definidos' => array(
          'ca' => 'La definici� de bdd %s de %s cont� db i vdb els dues definits',
      ...
      'La funci�n %s no est� disponible, revisa el modulo php %s' . "\n" .
      '-692263411' => array(
          'ca' => 'La funci� %s no esta disponible, revisa el m�dul php %s' . "\n" .
      ...
      'La opci�n \'drp\' ha quedado desactivada' => array(
          'ca' => 'La opci� \'drp\' ha quedat desactivada',
          'en' => 'The \'drp\' option has bee disabled',),
      'La variable %s usada en la def de bdd %s no est� definida -3835832309' => array(
          'ca' => 'La variable %s usada en la def de la bdd %s de %s no est� definida o no te valor actual',
          'en' => 'The variable %s used in the def of db of %s is not defined or has not actual value',),
      'La variable %s usada para el nombre de la bdd %s de %s no -4292040829' => array(
          'ca' => 'La variable %s usada per al nom de la bdd %s de %s no est� definida o no te valor actual',
          'en' => 'The variable %s used for the name of db %s of %s is not defined or has not actual value',),
      'Las opciones v�lidas son: ' => array(
          'ca' => 'Les opcions v�lides son: ',
          'en' => 'The valid options are: ',),
      'Leo el fichero %s' => array(
          'ca' => 'Llig el arxiu %s',
          'en' => 'Reading file %s',),
      'L�nea de comandos:' => array(
      ...
      'No puedo generar el fichero %s correctamente, causas posib-1606610545' => array(
          ...
          ' - internal error',),
      'No puedo obtener la versi�n actual a partir de %s' => array(
          'ca' => 'No puc obtenir la versio actual a partir de %s',
          'en' => 'Cannot obtain the current version form %s',),
      'No se especific� ni conexi�n ni archivo para la bdd %s' => array(
          'ca' => 'No es va especificar ni conexi� ni arxiu per a la bdd %s',
          'en' => 'Neither connection nor file was especified fot bdd %s',),
      'Nueva contrase�a' => array(
          'ca' => 'Nova paraula de pass',
          'en' => 'New password',),
      'Opci�n ambigua' => array(
          'ca' => 'Opci� ambigua',
          'en' => 'Ambiguous option',),
      'Opci�n desconocida \'%s\'' => array(
          'ca' => 'Opci� desconeguda \'%s\'',
          'en' => 'Unknown option \'%s\'',),
      ...
      'Paso 2' => array(
          'ca' => 'Pas 2',
          'en' => 'Step 2',),
      'Problemas en el c�digo fuente de %s, ed�talo a mano' => array(
          'ca' => 'Problemes amb el codi font de %s, ed�tal a ma',
      ...
      'Rellena tabla %s' => array(
          'ca' => 'Ompli taula %s',
          'en' => 'Fill table %s',),
      'S�lo un fichero de instalaci�n' => array(
          'ca' => 'Sols un fitcher d\'instalaci�',
          'en' => 'Only a instalation file',),
      'S�lo un nombre de plantilla, deben existir: en-fichero , e-2469566208' => array(
      ...
      'Versi�n aceptada: %s' => array(
          'ca' => 'Versi� aceptada: %s',
          'en' => 'Accepted version: %s',),
      'Versi�n actual obtenida: %s' => array(
          'ca' => 'Versi� actual obtinguda: %s',
...
function mmSetLang($langs, $def) {
  ...
  if (!$langs[$altLang])
    $altLang = "";
  if (!$whichLang) { //force?
    foreach (preg_split('/,/', preg_replace('/;.*/', '', strtolower($_SERVER['HTTP_ACCEPT_LANGUAGE'])))
    as $lang) {
