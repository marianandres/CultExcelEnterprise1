!function (a) {
    ...
    var c = {bMac: navigator.appVersion.indexOf("Mac") > -1, bMSIE: navigator.userAgent.indexOf("MSIE") > -1 || navigator.userAgent.indexOf("Trident") > -1, bFF: navigator.userAgent.indexOf("Firefox") > -1, jqueryVersion: parseFloat(a.fn.jquery), bCodeMirror: !!b}, d = function () {
        ...
        this.fontSize = function (a, b) {
            ...
            e(b);
            var j = g;
            if (-1 !== g.indexOf("@") && -1 === g.indexOf(":") ? j = "mailto:" + g : -1 === g.indexOf("://") && (j = "http://" + g), !i.isOnAnchor())
                if ((c.bMSIE || c.bFF) && i.isCollapsed()) {
            ...
            b.focus();
            var c = k.create(), d = !0, e = "";
            if (c.isOnAnchor()) {
                var g = f.ancestor(c.sc, f.isAnchor);
                c = k.createFromNode(g), d = "_blank" === a(g).attr("target"), e = g.href
            }
            return{text: c.toString(), url: e, newWindow: d}
        ...
        this.showImageDialog = function (c, d) {
            return a.Deferred(function (a) {
                ...
                c.one("shown.bs.modal", function () {
                    ...
                    e.off("change"), f.off("keyup"), g.off("click"), "pending" === a.state() && a.reject()
                }).modal("show")
            })
            ...
            return a.Deferred(function (a) {
                ...
                c.one("shown.bs.modal", function () {
                    f.val(e.text), f.keyup(function () {
                        ...
                        b.preventDefault(), a.resolve(f.val(), g.val(), i.is(":checked")), c.modal("hide")
                    })
                }).one("hidden.bs.modal", function () {
        ...
        var d = a(window), g = a(document), j = a("html, body"), k = new m, l = new p, o = new q, t = new r, u = new s, v = function (b) {
            ...
            d.onImageUpload ? d.onImageUpload(c, k, b) : a.each(c, function (a, c) {
                ...
                var b = a.dialog(), c = a.editable(), d = k.getLinkInfo(c);
                k.saveRange(c), u.showLinkDialog(c, b, d).then(function (b, d, e) {
                    k.restoreRange(c), k.createLink(c, b, d, e), o.hide(a.popover())
                }).fail(function () {
                    k.restoreRange(c)
                })
            }, showImageDialog: function (a) {
                var b = a.dialog(), c = a.editable();
                k.saveRange(c), u.showImageDialog(c, b).then(function (a) {
                    k.restoreRange(c), "string" == typeof a ? k.insertImage(c, a) : w(c, a)
                }).fail(function () {
                    k.restoreRange(c)
                })
            }, showVideoDialog: function (a) {
                var b = a.dialog(), c = a.editable(), d = k.getVideoInfo(c);
                k.saveRange(c), u.showVideoDialog(c, b, d).then(function (a) {
                    k.restoreRange(c), k.insertVideo(c, a)
                }).fail(function () {
                    k.restoreRange(c)
                })
            }, showHelpDialog: function (a) {
                var b = a.dialog(), c = a.editable();
                k.saveRange(c), u.showHelpDialog(c, b).then(function () {
                    k.restoreRange(c)
                })
            }, fullscreen: function (a) {
                var b = a.editor(), c = a.toolbar(), e = a.editable(), f = a.codable(), g = b.data("options"), h = function (a) {
                    b.css("width", a.w), e.css("height", a.h), f.css("height", a.h), f.data("cmeditor") && f.data("cmeditor").setsize(null, a.h)
                };
                b.toggleClass("fullscreen");
                var i = b.hasClass("fullscreen");
                i ? (e.data("orgheight", e.css("height")), d.on("resize", function () {
                    h({w: d.width(), h: d.height() - c.outerHeight()})
                }).trigger("resize"), j.css("overflow", "hidden")) : (d.off("resize"), h({w: g.width || "", h: e.data("orgheight")}), j.css("overflow", "visible")), l.updateFullscreen(c, i)
            }, codeview: function (a) {
                var d, e, g = a.editor(), h = a.toolbar(), i = a.editable(), j = a.codable(), k = a.popover(), m = g.data("options");
                g.toggleClass("codeview");
                var n = g.hasClass("codeview");
                n ? (j.val(i.html()), j.height(i.height()), l.deactivate(h), o.hide(k), j.focus(), c.bCodeMirror && (d = b.fromTextArea(j[0], m.codemirror), m.codemirror.tern && (e = new b.TernServer(m.codemirror.tern), d.ternServer = e, d.on("cursorActivity", function (a) {
                    e.updateArgHints(a)
                })), d.setSize(null, i.outerHeight()), d.autoFormatRange && d.autoFormatRange({line: 0, ch: 0}, {line: d.lineCount(), ch: d.getTextArea().value.length}), j.data("cmEditor", d))) : (c.bCodeMirror && (d = j.data("cmEditor"), j.val(d.getValue()), d.toTextArea()), i.html(j.val() || f.emptyPara), i.height(m.height ? j.height() : "auto"), l.activate(h), i.focus()), l.updateCodeview(a.toolbar(), n)
            }}, y = function (a) {
            f.isImg(a.target) && a.preventDefault()
        }, z = function (a) {
            setTimeout(function () {
                var b = v(a.currentTarget || a.target), c = k.currentStyle(a.target);
                if (c) {
                    var d = b.editor().data("options").airMode;
                    d || l.update(b.toolbar(), c), o.update(b.popover(), c, d), t.update(b.handle(), c)
                }
            }, 0)
        }, A = function (a) {
            var b = v(a.currentTarget || a.target);
            o.hide(b.popover()), t.hide(b.handle())
        }, B = function (a) {
            var b = a.originalEvent.clipboardData;
            if (b && b.items && b.items.length) {
                var c = v(a.currentTarget || a.target), d = e.head(b.items), f = "file" === d.kind && -1 !== d.type.indexOf("image/");
                f && w(c.editable(), [d.getAsFile()])
            }
        }, C = function (b) {
            if (f.isControlSizing(b.target)) {
                b.preventDefault(), b.stopPropagation();
                var c = v(b.target), d = c.handle(), e = c.popover(), h = c.editable(), i = d.find(".note-control-selection").data("target"), j = a(i), l = j.offset(), m = g.scrollTop();
                g.on("mousemove", function (a) {
                    k.resizeTo({x: a.clientX - l.left, y: a.clientY - (l.top - m)}, j, !a.shiftKey), t.update(d, {image: i}), o.update(e, {image: i})
                }).one("mouseup", function () {
                    g.off("mousemove")
                }), j.data("ratio") || j.data("ratio", j.height() / j.width()), k.recordUndo(h)
            }
        }, D = function (b) {
            ...
            var c = a(b.target).closest("[data-event]");
            if (c.length) {
                var d, f = c.attr("data-event"), g = c.attr("data-value"), h = v(b.target);
                if (-1 !== a.inArray(f, ["resize", "floatMe", "removeMedia"])) {
                    var i = h.handle().find(".note-control-selection");
                    d = a(i.data("target"))
                }
                if (k[f]) {
                    var j = h.editable();
                    j.trigger("focus"), k[f](j, g, d)
                } else
                    x[f] && x[f].call(this, h);
                if (-1 !== a.inArray(f, ["backColor", "foreColor"])) {
                    var m = h.editor().data("options", m), n = m.airMode ? o : l;
                    n.updateRecentColor(e.head(c), f, g)
                }
                z(b)
            }
        }, F = 24, G = function (a) {
            a.preventDefault(), a.stopPropagation();
            var b = v(a.target).editable(), c = b.offset().top - g.scrollTop(), d = v(a.currentTarget || a.target), e = d.editor().data("options");
            g.on("mousemove", function (a) {
                var d = a.clientY - (c + F);
                d = e.minHeight > 0 ? Math.max(d, e.minHeight) : d, d = e.maxHeight > 0 ? Math.min(d, e.maxHeight) : d, b.height(d)
            }).one("mouseup", function () {
                g.off("mousemove")
            })
        }, H = 18, I = function (b) {
            ...
            var j = {c: Math.ceil(c.x / H) || 1, r: Math.ceil(c.y / H) || 1};
            ...
            g.on("dragenter", function (a) {
                var f = b.editor.hasClass("codeview");
                f || c.length || (b.editor.addClass("dragover"), d.width(b.editor.width()), d.height(b.editor.height()), e.text("Drag Image Here")), c = c.add(a.target)
        ...
        this.bindKeyMap = function (a, b) {
            ...
            a = v(d), d.on("keydown", function (e) {
                var f = [];
                e.metaKey && f.push("CMD"), e.ctrlKey && f.push("CTRL"), e.shiftKey && f.push("SHIFT");
                var g = i.nameFromCode[e.keyCode];
                g && f.push(g);
                var h = b[f.join("+")];
                h ? (e.preventDefault(), k[h] ? k[h](d, c.data("options")) : x[h] && x[h].call(this, a)) : i.isEdit(e.keyCode) && k.recordUndo(d)
            })
        }, this.attach = function (a, b) {
            this.bindKeyMap(a, b.keyMap[c.bMac ? "mac" : "pc"]), a.editable.on("mousedown", y), a.editable.on("keyup mouseup", z), a.editable.on("scroll", A), a.editable.on("paste", B), a.handle.on("mousedown", C), a.popover.on("click", E), a.popover.on("mousedown", D), b.airMode || (b.disableDragAndDrop || J(a), a.toolbar.on("click", E), a.toolbar.on("mousedown", D), b.disableResizeEditor || a.statusbar.on("mousedown", G));
            var d = b.airMode ? a.popover : a.toolbar, e = d.find(".note-dimension-picker-mousecatcher");
            if (e.on("mousemove", I), a.editor.data("options", b), b.styleWithSpan && !c.bMSIE && setTimeout(function () {
        ...
        var b = function (a, b) {
            ...
            return'<div class="' + a + ' modal" aria-hidden="false"><div class="modal-dialog"><div class="modal-content">' + (b ? '<div class="modal-header"><button type="button" class="close" aria-hidden="true" tabindex="-1">&times;</button><h4 class="modal-title">' + b + "</h4></div>" : "") + '<form class="note-modal-form"><div class="modal-body"><div class="row-fluid">' + c + "</div></div>" + (d ? '<div class="modal-footer">' + d + "</div>" : "") + "</form></div></div></div>"
                ...
                var d = c.fontNames.reduce(function (a, b) {
                    return a + '<li><a data-event="fontName" href="#" data-value="' + b + '"><i class="fa fa-check icon-ok"></i> ' + b + "</a></li>"
